...
HANDLE intel_driver::Load()
{
	...
	std::cout << XorString(" [<] Loading Driver") << std::endl;
	...
	return CreateFileW(XorWideString(L"\\\\.\\Nal"), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
}

void intel_driver::Unload(HANDLE device_handle)
{
	std::cout << XorString(" [+] Cleaning Traces") << std::endl;
	Sleep(4500);
	Beep(667, 150);
	std::cout << XorString(" [>] Cleaned") << std::endl;
	Sleep(2500);

	std::cout << XorString(" [<] Unloading Driver\n") << std::endl;
	...
	const uint32_t get_temp_path_ret = GetTempPathA(sizeof(temp_directory), temp_directory);
	const std::string driver_path = std::string(temp_directory) + XorString("\\") + driver_name;
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size)
{
	...
	{
		std::cout << XorString(" [!] Failed To Translate Virtual Address 0x") << reinterpret_cast<void*>(address) << std::endl;
		return false;
	}
	...
	{
		std::cout << XorString(" [!] Failed To Map IO Space Of 0x") << reinterpret_cast<void*>(physical_address) << std::endl;
		return false;
	}
	...
	{
		std::cout << XorString(" [!] Failed To Unmap IO Space Of Physical Address 0x") << reinterpret_cast<void*>(physical_address) << std::endl;
	}
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size)
{
	...
	if (!kernel_ExAllocatePool)
		kernel_ExAllocatePool = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(XorString("ntoskrnl.exe")), XorString("ExAllocatePool"));
...
bool intel_driver::FreePool(HANDLE device_handle, uint64_t address)
{
	...
	if (!kernel_ExFreePool)
		kernel_ExFreePool = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(XorString("ntoskrnl.exe")), XorString("ExFreePool"));
...
bool intel_driver::GetNtGdiDdDDIReclaimAllocations2KernelInfo(HANDLE device_handle, uint64_t * out_kernel_function_ptr, uint64_t * out_kernel_original_function_address)
{
	...
	{
		const uint64_t kernel_NtGdiDdDDIReclaimAllocations2 = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(XorString("win32kbase.sys")), XorString("NtGdiDdDDIReclaimAllocations2"));
...
bool intel_driver::GetNtGdiGetCOPPCompatibleOPMInformationInfo(HANDLE device_handle, uint64_t * out_kernel_function_ptr, uint8_t * out_kernel_original_bytes)
{
	...
	{
		const uint64_t kernel_NtGdiGetCOPPCompatibleOPMInformation = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(XorString("win32kfull.sys")), XorString("NtGdiGetCOPPCompatibleOPMInformation"));
